...
class CodeWriter:
    ...
    def write_label(self, label: str) -> None:
        ...
        """
        self.out_stream.write('(' + self.function + '$' + label + ')')
    ...
    def write_goto(self, label: str) -> None:
        ...
        command = ['@' + self.function + '$' + label, '0;JMP']
        for i in command:
            self.out_stream.write(i)
            self.out_stream.write('\n')
    ...
    def write_if(self, label: str) -> None:
        ...
        # This is irrelevant for project 7,
        # you will implement this in project 8!
        command = ['@SP', 'M=M-1', '@SP', 'A=M', 'D=M', '@' + self.function + '$' + label, 'D;JNE']
        for c in command:
    ...
    def write_function(self, function_name: str, n_vars: int) -> None:
        ...
        self.out_stream.write('(' + self.function + ')' + '\n')
        counter = 0
    ...
    def write_call(self, function_name: str, n_args: int) -> None:
        ...
        self.function=function_name
        commands = ['@' + self.function + '$ret.' + str(CodeWriter.func_counter), 'D=A', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1'
            , '@1', 'D=M', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1', '@2', 'D=M', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1',
                    '@3', 'D=M', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1', '@4', 'D=M', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1',
                    '@SP', 'D=M','@5','D=D-A'
            , '@' + str(n_args), 'D=D-A', '@2', 'M=D', '@SP', 'D=M', '@1', 'M=D',
                    '@' + self.function, '0;JMP',
                    '(' + self.function + '$ret.' + str(CodeWriter.func_counter) + ')']
        for com in commands:
            self.out_stream.write(com)
            self.out_stream.write('\n')
        CodeWriter.func_counter += 1
    ...
    def write_return(self) -> None:
        ...
        command = ['@1', 'D=M', '@frame', 'M=D', '@5', 'D=A', '@frame', 'D=M-D', 'A=D', 'D=M',
                   '@return_address', 'M=D', '@SP', 'AM=M-1', 'D=M', '@2',
                   'A=M', 'M=D',
                   '@2', 'M=M+1', 'D=M', '@SP', 'M=D',
                   '@frame', 'M=M-1', 'A=M', 'D=M', '@4', 'M=D',
                   '@frame', 'M=M-1', 'A=M', 'D=M', '@3', 'M=D',
                   '@frame', 'M=M-1', 'A=M', 'D=M', '@2', 'M=D',
                   '@frame', 'M=M-1', 'A=M', 'D=M', '@1', 'M=D'
            , '@return_address' , 'A=M', '0;JMP']
        for i in command:
            self.out_stream.write(i)
            self.out_stream.write('\n')
